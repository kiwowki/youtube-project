export const todayText = [
    {
        img: "https://i.ytimg.com/vi/tI-5uv4wryI/hqdefault.jpg",
        title: "Oasis - Champagne Supernova (Official Video)",
        desc: (
            <>
                "Champagne Supernova"는 1990년대의 브리티시 록 신에서 특히 돋보이는 작품 중 하나로, 그 당시의 음악적 트렌드를 잘 반영하고 있습니다. "Champagne Supernova"는 그들만의 독특한 감성과 진정성으로 많은 리스너들에게 여운을 남기고 있는 곡 중 하나입니다.
                매력적인 멜로디와 감동적인 가사를 한번 느껴보세요!
            </>
        ),
        author: "Oasis",
        date: "2014. 1. 29",
        videoId: "tI-5uv4wryI",
        channelId: "UCUDVBtnOQi4c7E8jebpjc9Q"
    },
    {
        img: "https://i.ytimg.com/vi/45cYwDMibGo/hqdefault.jpg",
        title: "The Beatles - Come Together",
        desc: "The Beatles performing “Come Together.” 귀여운 애니메이션과 함께하는 'Come Together'를 즐겨보세요.",
        author: "The Beatles",
        date: "2018. 06. 22",
        videoId: "45cYwDMibGo",
        channelId: "UCc4K7bAqpdBP8jh1j9XZAww"
    },
    {
        img: "https://i.ytimg.com/vi/Bf01riuiJWA/hqdefault.jpg",
        title: "Exit Music (For A Film)",
        desc: (
            <>
                Exit Music (For A Film) · Radiohead<br /><br />

                OK Computer

                ℗ 1997 XL Recordings Ltd

                Released on: 1997-05-28
            </>
        ),
        author: "Radiohead",
        date: "2016. 4. 7",
        videoId: "Bf01riuiJWA",
        channelId: "UCq19-LqvG35A-30oyAiPiqA"
    },
    {
        img: "https://i.ytimg.com/vi/tH-GPdkkNik/hqdefault.jpg",
        title: "What If",
        desc: (
            <>
                Provided to YouTube by Parlophone Records Limited<br /><br /> What If · Coldplay

                X & Y

                ℗ 2005 Parlophone Records Limited
            </>
        ),
        author: "Coldplay",
        date: "2015. 9. 11",
        videoId: "tH-GPdkkNik",
        channelId: "UCDPM_n1atn2ijUwHd0NNRQw"
    }, ,
    {
        img: "https://i.ytimg.com/vi/hTWKbfoikeg/hqdefault.jpg",
        title: "Nirvana - Smells Like Teen Spirit (Official Music Video)",
        desc: (
            <>
                Official Music Video for Smells Like Teen Spirit performed by Nirvana.<br /><br /> Commemorate 30 years of 'Nevermind' with the album newly remastered from the original half-inch stereo analog tapes plus four previously unreleased international live shows from Amsterdam, Netherlands; Del Mar, California; Melbourne, Australia and Tokyo, Japan.
            </>
        ),
        author: "Nirvana",
        date: "2009. 6. 17",
        videoId: "hTWKbfoikeg",
        channelId: "UCFMZHIQMgBXTSxsr86Caazw"
    }, ,
    {
        img: "https://i.ytimg.com/vi/ujNeHIo7oTE/hqdefault.jpg",
        title: "U2 - With Or Without You (Official Music Video)",
        desc: (
            <>
                REMASTERED IN HD!<br />
                The official music video for With Or Without You by U2.
                Filmed at Ardmore Studios in Bray, just outside Dublin, by Meiert Avis, this video for U2's first US number 1 single released in March 1987 features additional footage by Matt Mahurin.
            </>
        ),
        author: "U2",
        date: "2019. 7. 16",
        videoId: "ujNeHIo7oTE",
        channelId: "UC4gPNusMDwx2Xm-YI35AkCA"
    }, ,
    {
        img: "https://i.ytimg.com/vi/dbB-mICjkQM/hqdefault.jpg",
        title: "Muse - Plug In Baby (Official Video)",
        desc: (
            <>
                Watch the official music video for "Plug In Baby" now!<br /><br />

                WILL OF THE PEOPLE - The new album released 26th August 2022
            </>
        ),
        author: "Muse",
        date: "2010. 2. 9",
        videoId: "dbB-mICjkQM",
        channelId: "UCGGhM6XCSJFQ6DTRffnKRIw"
    },
]